...
import { Card, Col, Button, Breadcrumb } from "@themesberg/react-bootstrap";
...
const LayananSecurityProvider = () => {
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                await axios.delete(`${process.env.REACT_APP_API_PWA_TNOS_DSBRD_URL}/dashboard/pwa-revamp/layanan/${id}`);
                fetchData();
                Swal.fire(
                    'Berhasil!',
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_API_PWA_TNOS_DSBRD_URL}/pwa-revamp/layanan/${id_provider}`);
            setLayanan(response.data.data.layanan);
        } catch (error) {
